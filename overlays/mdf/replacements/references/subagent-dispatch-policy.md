...
2. Inspect the available runtime capability and invocation paths. A candidate
   is eligible only when the runtime can identify its GPT-5.6 family, variant,
   and supported effort. MDF does not assume a measured quality/cost registry;
   model quality and cost trade-offs are the root model's judgment.
...
4. Select a candidate dynamically from the GPT-5.6 frontier, applying the
   quality floor first and then judging quality, cost, ambiguity, novelty, and
   consequence together. High-risk work should prefer the candidate the root
   judges most capable, even when a cheaper candidate appears attractive.
...
model or effort. If the root cannot identify an eligible GPT-5.6 capability or
cannot complete the record, stop or use the root fallback with `degraded: true`.
...
candidate family is strictly GPT-5.6: do not silently downgrade to GPT-5.5 or
adopt an unreviewed future profile.

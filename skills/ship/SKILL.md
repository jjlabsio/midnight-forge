...
1. Resolve the installed plugin root and persona prompts. Upstream permits
   project or global persona precedence. MDF-managed portable dispatch uses the
   exact plugin prompt required by the dispatch policy when native scoped-agent
   resolution is unavailable; disclose the adaptation.
...
4. The root directly performs upstream fan-out and merge. Do not add a ship
   worker, critic, verifier, or coordinator.
5. Do not push, create a PR, merge, deploy, delete, or mutate task lifecycle;
   those require a separately authorized caller.

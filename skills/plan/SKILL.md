...
6. Bind the plan to the current autonomous execution envelope after review
...
MDF port: the upstream human sign-off checklist is represented by the current
autonomous authority binding, exact artifact hashes, and applicable Two-Key
`PASS`; it does not create a human permission checkpoint. Preserve all other
upstream planning quality and Definition-of-Done requirements.
...
authority or lifecycle rules. A bare mode string is not authority. In every
mode, require the exact specification revision to be bound to the current
autonomous envelope before planning.
...
vertical-slice, risk, checkpoint, Definition of Done, or review sequence.

Load the shared `../../references/approval-evidence.md` before applying this
skill's authority gates. It owns the common exact-artifact path/hash, authority
binding, and invalidation rules.

## Create or revise a plan

1. Read the current specification by its exact path and SHA-256, relevant
...
2. Map the dependency graph, then break the work into small vertical slices
   ...
   acceptance criteria, verification commands, and any autonomous authority or
   external-state stop.
...
4. Write a checklist-style new revision under the canonical work item as
   `.mdf/work/<work-id>/plan-NNN.md`; never patch an existing revision in place.
...
5. Review the complete plan against the current specification and upstream
   planning verification and Definition of Done. Fix actionable findings
   before binding it to the current authority envelope.
...
1. The root supplies the exact current specification bytes, path, and hash,
...
4. A distinct fresh-context, read-only, non-delegating verifier assesses those
   actual bytes against the exact current specification, original planning
...
planning requires the exact current specification revision and plan revision to
be bound to the autonomous execution envelope before implementation. A review
pass or a saved plan is not authority by itself. In automatic modes, use the
current handoff from the loaded auto-workflow contract only after a Two-Key
`PASS`; this is integrity and stage evidence, not a repeated human checkpoint.
Any byte, path, scope, or task-order change requires a new revision and
invalidates the auto handoff and affected downstream evidence.

If either exact artifact is missing, stale, ambiguous, or outside the envelope,
stop before build as `BLOCKED` and report the evidence. In automatic modes,
continue only under the current auto-workflow handoff after its dependency and
critical-decision checks pass. Do not silently continue from an invalidated
plan revision or ask for routine approval.

## Revisions and handoff

A plan-only revision is valid only for a dependency, order, owned-path, or task
scope representation defect while the current specification and its material
...
redoing work or committing automatically. Hand off to `build` only after the
exact revisions are bound to the autonomous envelope, automatic Two-Key `PASS`
when applicable, and all dependency, checkpoint, and critical-decision stops
are clear.

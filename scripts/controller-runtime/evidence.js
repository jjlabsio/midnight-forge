...
function assertCanonicalEvidenceAncestors(boundary, candidate) {
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    if (stat?.isSymbolicLink()) throw new ControllerError("MDF_EVIDENCE_SYMLINK", "Evidence paths must not contain symlinked ancestors.", { path: current });
  }
}
...
function evidencePath(context, relativePath) {
  ...
  assertCanonicalEvidenceAncestors(context.work_item.path, candidate);
...
function nextFile(context, kind, filePrefix = kind) {
  ...
  return { directory, file: `${filePrefix}-${String(count).padStart(3, "0")}.json` };
}

function write(context, kind, value, filePrefix = kind) {
  const target = nextFile(context, kind, filePrefix);
...
function resolveRemoteBase(context, code = "MDF_REVIEW_BASE_INVALID") {
  const symbolic = spawnSync("git", ["symbolic-ref", "--quiet", "refs/remotes/origin/HEAD"], { cwd: context.worktree, encoding: "utf8" });
  let branch = null;
  ...
  if (!branch) {
    const remote = spawnSync("git", ["remote", "show", "origin"], { cwd: context.worktree, encoding: "utf8" });
    const match = remote.stdout?.match(/^\s*HEAD branch:\s*(\S+)\s*$/m);
    if (remote.status !== 0 || !match) throw new ControllerError(code, "Could not resolve origin default branch.", { stderr: remote.stderr });
    branch = match[1];
  }
  const baseRef = `origin/${branch}`;
  const resolved = spawnSync("git", ["rev-parse", "--verify", `${baseRef}^{commit}`], { cwd: context.worktree, encoding: "utf8" });
  if (resolved.status !== 0 || !resolved.stdout.trim()) throw new ControllerError(code, "Could not resolve the remote base commit.", { ref: baseRef, stderr: resolved.stderr });
  return { base_ref: baseRef, base_commit: resolved.stdout.trim() };
}
...
function recordVerification(context, { invocation, input_paths: inputPaths }) {
  if (!invocation || typeof invocation.agent_id !== "string" || typeof invocation.invocation_id !== "string" || !Array.isArray(inputPaths)) {
    throw new ControllerError("MDF_INTERACTION_INVALID", "Verification interactions require an agent invocation and exact input paths.");
  }
  return write(context, "interaction", { invocation, inputs: inputPaths.map((inputPath) => fact(context, inputPath)), git: computeGitFacts(context) }, "verification");
}
...
function assertFact(context, value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value) || typeof value.path !== "string" || !value.path || path.isAbsolute(value.path) || value.path.includes("\\") || path.posix.normalize(value.path) !== value.path || value.path === ".." || value.path.startsWith("../") || !/^[a-f0-9]{64}$/.test(value.sha256 || "") || !Number.isInteger(value.bytes) || value.bytes < 0) {
    throw new ControllerError("MDF_EVIDENCE_FABRICATED", "Evidence fact has an invalid shape.", { label });
  }
  const candidate = path.resolve(context.work_item.path, value.path);
  if (!inside(context.work_item.path, candidate)) throw new ControllerError("MDF_EVIDENCE_PATH_ESCAPE", "Evidence fact path escapes the work item.", { label, path: value.path });
  assertCanonicalEvidenceAncestors(context.work_item.path, candidate);
  const stat = fs.lstatSync(candidate, { throwIfNoEntry: false });
  if (stat && (!stat.isFile() || stat.isSymbolicLink())) throw new ControllerError("MDF_EVIDENCE_FABRICATED", "Evidence fact path must resolve to a regular file.", { label, path: value.path });
}

function assertGitFacts(value, label) {
  if (!value || typeof value !== "object" || !/^[a-f0-9]{40}$/.test(value.head || "") || !/^[a-f0-9]{64}$/.test(value.status_sha256 || "") || !/^[a-f0-9]{64}$/.test(value.worktree_sha256 || "")) {
    throw new ControllerError("MDF_EVIDENCE_FABRICATED", "Git evidence facts have an invalid shape.", { label });
  }
}
...
function verifySidecar(context, file, { fresh = true } = {}) {
  ...
  if (sidecar.kind === "artifact") { assertFact(context, sidecar.artifact, "artifact"); if (fresh) matches(context, sidecar.artifact); }
  else if (sidecar.kind === "command") { if (!Array.isArray(sidecar.command) || !Number.isInteger(sidecar.exit_code)) throw new ControllerError("MDF_EVIDENCE_FABRICATED", "Command sidecar lacks facts.", { file }); assertFact(context, sidecar.output, "command.output"); if (fresh) matches(context, sidecar.output); }
    ...
    if (!Array.isArray(sidecar.inputs)) throw new ControllerError("MDF_EVIDENCE_FABRICATED", "Interaction sidecar lacks input facts.", { file });
    sidecar.inputs.forEach((input) => assertFact(context, input, "interaction.input"));
    assertGitFacts(sidecar.git, "interaction.git");
    if (fresh) {
      sidecar.inputs.forEach((input) => matches(context, input));
    ...
    assertGitFacts(sidecar.git, "git");
...
module.exports = { recordArtifact, recordCommand, recordInteraction, recordVerification, recordDecision, recordGitFacts, resolveRemoteBase, verifyInputs, verifySidecar };

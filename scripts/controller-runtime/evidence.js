...
function recordGitFacts(context) {
  ...
  return write(context, "git", { git: { head, status_sha256: sha256(status) } });
}
...
function recordInteraction(context, { invocation, input_paths: inputPaths }) {
  ...
  return write(context, "interaction", { invocation, inputs: inputPaths.map((inputPath) => fact(context, inputPath)), git: recordGitFacts(context).git });
}
...
function verifySidecar(context, file) {
  ...
  if (sidecar.kind === "artifact") matches(context, sidecar.artifact);
    ...
    const current = recordGitFacts(context).git;

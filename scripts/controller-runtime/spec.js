...
function registerSpec(context, { artifact_path: artifactPath, review_output_path: reviewPath, review_decision_file: reviewDecisionFile, mode }) {
  ...
  const registration = recordInteraction(context, { invocation: { agent_id: "mdf-spec", invocation_id: `spec-${artifact.file}`, executor: "deterministic-runtime", mode, artifact_file: artifact.file, review_decision_file: reviewDecisionFile }, input_paths: [artifactPath, reviewPath, `evidence/${reviewDecisionFile}`] });
...
function approveSpec(context, { registration_file: registrationFile, user_message_path: userMessagePath, invocation_id: invocationId }) {
